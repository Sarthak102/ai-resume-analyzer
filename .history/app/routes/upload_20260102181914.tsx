...
const ensureFullAuth = async (auth: any) => {
  if (typeof window === "undefined") return false;

  const puter = window.puter;
  if (!puter) return false;

  // 🚫 Temp users cannot upload
  if (puter.whoami?.is_temp) {
    await auth.signIn(); // force full login
    return false;
  }
...
const Upload = () => {
  ...
  /* 🔐 STEP 1: FORCE FULL AUTH */
  setStatusText("Checking authentication...");
  const isAuthed = await ensureFullAuth(auth);
  ...
  if (!imageFile?.file) {
    setStatusText("Error: Failed to convert resume");
    setIsProcessing(false);
    return;
  }
